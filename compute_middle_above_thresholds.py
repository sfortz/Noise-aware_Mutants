...
def get_min_values():
    ...
    min_values = c.get_tolerance_values('N', 'brisbane')

    # Find the minimum value for each metric
    for hw in c.hardware:
        for metric, value in c.get_tolerance_values('N', hw).items():
...
def get_max_values():
    # Initialize the result dictionary
    max_values = c.get_tolerance_values('N', 'brisbane')
...
def get_middle_threshold():
    ...
    for metric in c.metrics.values():
        if metric == 'fidelity':
            middle_threshold[metric] = np.median([max_values[metric], c.get_tolerance_values('I')[metric]])
        else:
            middle_threshold[metric] = np.median([min_values[metric], c.get_tolerance_values('I')[metric]])
